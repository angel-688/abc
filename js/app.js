(() => {
    ...
    function formFieldsInit(options = {
        ...
        if (options.inputValidate) document.body.addEventListener("input", (function(e) {
            const targetElement = e.target;
            if (targetElement.tagName === "INPUT" && targetElement.dataset.required !== "email" && targetElement.dataset.required !== "") {
    ...
    let formValidate = {
        getErrors(form) {
            ...
            if (formRequiredItems.length) formRequiredItems.forEach((formRequiredItem => {
                if ((formRequiredItem.offsetParent !== null || formRequiredItem.tagName === "SELECT") && !formRequiredItem.disabled) if (forms_status) error += this.validateInput(formRequiredItem) + this.inputValidate(formRequiredItem, true); else error += this.validateInput(formRequiredItem);
            }));
            return error;
        },
        ...
        inputValidate(formRequiredItem, start = false) {
            if (!start) return;
            const isText = formRequiredItem.dataset.required === "text";
            const isTel = formRequiredItem.dataset.required === "tel";
            let error = 0;
            ...
            error === 0 ? this.removeInputError(formRequiredItem) : this.addInputError(formRequiredItem);
            return error;
        },
    ...
    function formRating() {
        ...
        function initRatings() {
            ...
            function initRatingVars(rating) {
                ratingActive = rating.querySelector(".rating__active") ? rating.querySelector(".rating__active") : null;
                ratingValue = rating.querySelector(".rating__value");
            }
            function setRatingActiveWidth(rating, index = ratingValue.innerHTML, hover = false) {
                if (rating.classList.contains("rating--custom")) {
                    const ceilNum = Math.ceil(+index);
                    const fractionNum = Math.round(+index % 1 * 10) * 10;
                    const allClip = document.querySelectorAll('[id^="star-clip"]');
                    const id = allClip.length ? `star-clip-${allClip.length}` : "star-clip";
                    const ratingIcons = rating.querySelectorAll(".rating__icon");
                    const cut = rating.querySelector("._cut");
                    removeClasses(ratingIcons);
                    if (cut) cut.classList.remove("_cut");
                    for (let i = 0; i < ceilNum; i++) {
                        ratingIcons[i].classList.add("_rating-active");
                        if (i + 1 === ceilNum && fractionNum) {
                            ratingIcons[i].classList.add("_cut");
                            addMask(rating);
                        }
                    }
                    if (hover) removeMask();
                    function addMask(rating) {
                        removeMask();
                        const cutItem = rating.querySelector(".rating__icon._cut svg");
                        cutItem.insertAdjacentHTML(`afterbegin`, `<defs>\n\t\t\t\t\t\t<clipPath id="${id}">\n\t\t\t\t\t\t\t<rect x="0" y="0" width="${fractionNum}%" height="100%" />\n\t\t\t\t\t\t</clipPath>\n\t\t\t\t</defs>`);
                        const originalPath = cutItem.querySelector("path");
                        const newPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
                        newPath.setAttribute("d", originalPath.getAttribute("d"));
                        newPath.setAttribute("clip-path", `url(#${id})`);
                        cutItem.insertAdjacentElement(`beforeend`, newPath);
                    }
                    function removeMask() {
                        const mask = rating.querySelector(`[id^="star-clip"]`) ? rating.querySelector(`[id^="star-clip"]`).parentElement : null;
                        const clonPath = rating.querySelector(`path[clip-path^="url(#star-clip"]`);
                        if (mask && clonPath) {
                            mask.remove();
                            clonPath.remove();
                        }
                    }
                    function removeClasses(ratingIcons) {
                        for (let i = 0; i < ratingIcons.length; i++) if (ratingIcons[i].classList.contains("_rating-active")) ratingIcons[i].classList.remove("_rating-active");
                    }
                } else {
            ...
            function setRating(rating) {
                ...
                for (let index = 0; index < ratingItems.length; index++) {
                    ...
                    ratingItem.addEventListener("mouseenter", (function(e) {
                        initRatingVars(rating);
                        setRatingActiveWidth(rating, ratingItem.value, true);
                    }));
    ...
    class SelectConstructor {
        ...
        getSelectTitleValue(selectItem, originalSelect) {
            ...
            if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`; else {
                ...
                if (originalSelect.hasAttribute("data-svg")) return `<button type="button" class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span><svg class="select__icon"><use xlink:href="${originalSelect.dataset.svg}"></use></svg></span></button>`; else return `<button type="button" class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`;
            }
        }
        ...
        getOption(selectOption, originalSelect) {
            ...
            const selectOptionId = selectOption.dataset.optionId ? `${selectOption.dataset.optionId}` : "";
            const selectOptionLink = selectOption.dataset.href ? selectOption.dataset.href : false;
            const selectOptionLinkTarget = selectOption.hasAttribute("data-href-blank") ? `target="_blank"` : "";
            let selectOptionHTML = ``;
            if (selectOptionLink) selectOptionHTML += `<a ${selectOptionLinkTarget} ${selectOptionHide} href="${selectOptionLink}" data-value="${selectOption.value}" class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}">`; else if (selectOptionLink && selectOptionId) selectOptionHTML += `<a ${selectOptionLinkTarget} ${selectOptionHide} href="${selectOptionLink}" id="${selectOptionId}" data-value="${selectOption.value}" class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}">`; else if (!selectOptionLink && selectOptionId) selectOptionHTML += `<button ${selectOptionHide} id="${selectOptionId}" class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}" data-value="${selectOption.value}" type="button">`; else selectOptionHTML += `<button ${selectOptionHide} class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}" data-value="${selectOption.value}" type="button">`;
    ...
    function rangeInit() {
        ...
        const creditTimeRange = document.querySelector("#credit-time-range");
        if (priceSlider) {
            ...
            priceSlider.noUiSlider.on("update", (function(values, handle) {
                const parent = priceSlider.parentElement.parentElement;
                const minMax = [ parent.querySelector("[data-min]"), parent.querySelector("[data-max]") ];
                minMax[handle].textContent = values[handle];
            }));
        }
        if (creditSumRange) {
            creditSumRange.getAttribute("data-from");
            creditSumRange.getAttribute("data-to");
            initialize(creditSumRange, {
                start: [ 0 ],
                connect: "lower",
                range: {
                    min: [ 0 ],
            ...
            creditSumRange.noUiSlider.on("update", (function(values, handle) {
                const parent = creditSumRange.parentElement;
                const minMax = [ parent.querySelector("[data-range-value]") ];
                minMax[handle].textContent = values[handle];
                function calculator() {
                    const input = document.querySelector(".calculator-credit__input");
                    const initialPrice = document.querySelector("[data-initial]");
                    const loanPrice = document.querySelector("[data-loan]");
                    const fill = document.querySelector(".credit__fill span");
                    let creditSum = minMax[handle].textContent;
                    const format = wNumb({
                        decimals: 0,
                        thousand: " "
                    });
                    creditSum = getNum(creditSum);
                    if (input) input.addEventListener("input", (function(e) {
                        creditSum = getNum(minMax[handle].textContent);
                    ...
                    function changeValue() {
                        ...
                        if (value.length) {
                            ...
                            if (creditSum) {
                                fill.style.cssText = `\n\t\t\t\t\t\twidth: ${value / creditSum * 100 < 100 ? value / creditSum * 100 : 100}%;\n\t\t\t\t\t\ttransition-duration: 0.8s;\n\t\t\t\t\t\ttransition-property: width;`;
                                if (result > 0) {
                                    loanPrice.textContent = format.to(result);
                                    loanPrice.classList.add("_active");
                                } else {
                                    loanPrice.textContent = "0";
                                    loanPrice.classList.remove("_active");
                                }
                            }
                        } else {
                            fill.style.width = "0%";
                            initialPrice.textContent = "0";
                            loanPrice.textContent = "0";
                            initialPrice.classList.remove("_active");
                            loanPrice.classList.remove("_active");
                            creditSum = "0";
                        }
                    }
        ...
        if (creditTimeRange) {
            creditTimeRange.getAttribute("data-from");
            creditTimeRange.getAttribute("data-to");
            initialize(creditTimeRange, {
                start: 0,
                connect: "lower",
                range: {
                    min: [ 6, 12 ],
            ...
            creditTimeRange.noUiSlider.on("update", (function(values, handle) {
                const parent = creditTimeRange.parentElement;
                const minMax = [ parent.querySelector("[data-range-value]") ];
                minMax[handle].textContent = `${values[handle]} мес.`;
            }));
        }
    }
    rangeInit();
    ...
    function initSliders() {
        ...
        if (document.querySelector(".categories__slider")) new swiper_core_Swiper(".categories__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".arrow--categories.arrow--prev",
                nextEl: ".arrow--categories.arrow--next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.1,
        ...
        if (document.querySelector(".special__slider")) new swiper_core_Swiper(".special__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            speed: 800,
            navigation: {
                prevEl: ".arrow--special.arrow--prev",
                nextEl: ".arrow--special.arrow--next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.1,
        ...
        if (document.querySelector(".partners__slider")) new swiper_core_Swiper(".partners__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".arrow--partners.arrow--prev",
                nextEl: ".arrow--partners.arrow--next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.5,
        ...
        if (document.querySelector(".recomendation__slider")) new swiper_core_Swiper(".recomendation__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
            ...
            speed: 800,
            navigation: {
                prevEl: ".arrow--recom.arrow--prev",
                nextEl: ".arrow--recom.arrow--next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.2,
        ...
        if (document.querySelector(".archive__slider")) new swiper_core_Swiper(".archive__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
            ...
            speed: 800,
            navigation: {
                prevEl: ".arrow--archive.arrow--prev",
                nextEl: ".arrow--archive.arrow--next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
        ...
        if (document.querySelectorAll(".tabs__slider")) {
            let thumbs = [];
            if (window.innerWidth > 991.98) if (document.querySelectorAll(".thumbs")) document.querySelectorAll(".thumbs").forEach((thumb => {
                let thumbSlider = new swiper_core_Swiper(thumb, {
                ...
                thumbs.push(thumbSlider);
            }));
            document.querySelectorAll(".tabs__slider").forEach(((tabSlider, index) => {
                new swiper_core_Swiper(tabSlider, {
    ...
    function initYoutube() {
        const youTubeContainers = document.querySelectorAll("[data-youtube-video]");
        ...
        if (youTubeContainers.length) youTubeContainers.forEach((container => {
            const videoId = container.dataset.youtubeVideo;
            const videoParent = container.parentElement;
            videoParent.addEventListener("click", (() => {
                const iframe = createYouTubeIframe(videoId);
                container.innerHTML.trim() !== "" ? container.innerHTML = "" : null;
